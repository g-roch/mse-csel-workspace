...
// This is the global variable that stocks the data written
...
{
	// Print the major and minor number of the device being opened
    pr_info("skeleton : open operation... major:%d, minor:%d\n",
            imajor(i),
            iminor(i));

	// Print the access mode of the file being opened
                             ...
                             loff_t* off)
{
	...
	// - remaining bytes to copy is the total buffer size minus the offset
    ssize_t remaining = BUFFER_SZ - (ssize_t)(*off);
	// - ptr is the pointer to the buffer at the offset
    char* ptr         = s_buffer + *off;

	// - if the count of bytes to read is greater than the remaining bytes, update the count to the remaining bytes
    if (count > remaining) count = remaining;
    *off += count;

    // copy required number of bytes 
	// (from the internal buffer to the user buffer)
                              ...
                              loff_t* off)
{
	...
	// - if the count of bytes to write is greater than the remaining space, update the count to an error code
    ...
    if (count > 0) {
        ...
        *off += count;
		...
		// if the copy from user to kernel buffer fails, update the count to an error code
...
// This structure defines the file operations that the device supports
...
{
	// Allocate a device number for the character device
    int status = alloc_chrdev_region(&skeleton_dev, 0, 1, "mymodule");

	// If the allocation is successful, initialize the character device and add it to the system
    if (status == 0) {
...
{
	// Remove the character device from the system and unregister the device number
